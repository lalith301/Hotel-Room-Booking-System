...
exports.createRoom = async (req, res) => {
  try {
    ...
    // check `extra_facilities` filed exits (allow empty array)
    if (!extra_facilities || !Array.isArray(extra_facilities) || extra_facilities.length === 0) {
      cleanupFiles();
      return res.status(400).json(errorResponse(
        1,
        'FAILED',
    ...
    // check `req.files` filed exits - MAKE THIS OPTIONAL FOR ADMIN PANEL
    if (!req.files || !req.files[0]) {
      // For admin panel, we'll allow rooms without images initially
      // You can change this behavior later
      console.log('No files uploaded - creating room without images');
    }
    ...
    const data = {
      ...
      room_size: Number(room_size),
      room_capacity: Number(room_capacity),
      allow_pets: allow_pets === 'true' || allow_pets === true,
      provide_breakfast: provide_breakfast === 'true' || provide_breakfast === true,
      featured_room: featured_room === 'true' || featured_room === true,
      room_description,
      extra_facilities,
      room_images: req.files && req.files.length > 0 
        ? req.files.map((file) => ({ url: `/uploads/rooms/${file.filename}` }))
        : [{ url: '/uploads/rooms/default-room.jpg' }], // Default image if no files
...
exports.editRoomByAdmin = async (req, res) => {
  try {
    ...
    // Make file upload optional for edit - if no new files, keep existing images
    if (!req.files || !req.files[0]) {
      console.log('No new files uploaded - keeping existing images');
    }
